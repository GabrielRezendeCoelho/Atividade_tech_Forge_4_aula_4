...
router.post('/upload', (req, res) => {
  upload.array('meusArquivos', 10)(req, res, (err) => {
    if (err instanceof multer.MulterError) {
      if (err.code === 'LIMIT_FILE_SIZE') return res.status(400).json({ error: 'Arquivo muito grande. Tamanho máximo: 5MB.' });
      if (err.code === 'LIMIT_FILE_COUNT' || err.code === 'LIMIT_UNEXPECTED_FILE') return res.status(400).json({ error: 'Too many files' });
      return res.status(400).json({ error: err.message });
    }

    if (err) {
      if (err.code === 'INVALID_FILE_TYPE') return res.status(400).json({ error: 'Tipo de arquivo inválido.' });
